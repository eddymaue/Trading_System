...
workbook_path = r"C:\MesOutils\MesDocumentsOffice\Bourse\BourseV14rev024 Beta.xlsm"

excel_app = win32.Dispatch("Excel.Application")
wrkbk = excel_app.Workbooks.Open(workbook_path)
wrkbk.Parent.Application.Visible = True

if False:
    excel_app = win32.Dispatch("Excel.Application")
    wrkbk = excel_app.Workbooks.Add(workbook_path)
...
def do_move(event):
    appXC = wrkbk.Parent.Application
    lnCompensation = 1.3333
    dx = event.x - fenetre_modale.x
    dy = event.y - fenetre_modale.y
    appXC.left = appXC.left + (dx/lnCompensation)
    appXC.top  = appXC.top + (dy/lnCompensation)
    print(f"XLleft ={appXC.left} , XLtop ={appXC.top}")
    y =int( appXC.left * lnCompensation)
    x =int( appXC.top  * lnCompensation)
    print(f"PYleft ={x} , PYtop ={y}")
...
def executer_script():
    ...
    os.system(f'python MouseAutomation.py {chemin_fichier}')
...
if len(sys.argv) != 3 :
    ...
    try:
        ...
        leftValue = float(sys.argv[1])
        m = float(1.31)
        b= float(0)
        leftValue =int( m * leftValue +b)
